...
| `GH_TOKEN_FILE` | (unset) | GitHub 書き込み用トークン（プライマリ、自動で `GH_TOKEN` としてエクスポートされる）を保存したホスト側ファイルへの絶対パス（後述） |
| `GH_TOKEN_SECONDARY_FILE` | (unset) | GitHub 書き込み用トークン（セカンダリ、自動exportされずコマンドごとに明示指定する）を保存したホスト側ファイルへの絶対パス（後述） |
| `GITCONFIG_FILE` | (unset) | コンテナ内 `~/.gitconfig` として read-only マウントするホスト側 git 設定ファイルのパス（後述） |
| `SECRETS_DIR` | (unset) | 任意本数・任意名のトークンをコンテナへ持ち込む汎用シークレットディレクトリのホスト側パス（後述） |
...
**GitHub への書き込み（`GH_TOKEN_FILE` / `GH_TOKEN_SECONDARY_FILE`、legacy）**: コンテナ内から `gh` CLI で GitHub に書き込む（Issue作成、自リポジトリのPRレビュー等）場合、以下の手順でトークンを配線する。より汎用的な `SECRETS_DIR`（後述）でも同じ用途を賄えるが、「マウントするがコンテナ内で自動 export しない」というセカンダリ相当の運用は本節の `GH_TOKEN_SECONDARY_FILE` でのみ可能なため、当面はこちらも残している。

**重要**: 「`.claude-container.d/env` を置く場所」と「PATの `Repository access` で選ぶリポジトリ」は別物である。前者はトークンを**使う側**のプロジェクト（例: findsummits）、後者は書き込み先リポジトリ（例: claude-container）を指す。findsummits から claude-container の Issue に書き込みたい場合、`.claude-container.d/env` は findsummits 側に置き、PATの `Repository access` には `claude-container` を選択する — 自分自身（使う側）のリポジトリを登録するわけではない。

**プライマリとセカンダリの使い分け**: fine-grained PAT はトークン単位で、選択した全リポジトリに同一のパーミッションが一律適用される仕様である（リポジトリごとに異なるパーミッションは設定できない）。そのため「自分自身のリポジトリだけ広い権限（例: Pull requests も含む）、他のリポジトリは Issues のみ」としたい場合はトークンを2本に分ける必要がある。
- `GH_TOKEN_FILE`（プライマリ）: `entrypoint.sh` が自動的に `GH_TOKEN` としてエクスポートし `gh` CLI が自動認識する。対象・権限が1組で足りるならこれ1本で構わない
- `GH_TOKEN_SECONDARY_FILE`（セカンダリ、任意）: 自動エクスポートされない。使うコマンドの前に `GH_TOKEN=$(cat /home/node/.config/claude-container/gh-token-secondary) gh issue create --repo <owner>/<repo> ...` のように明示指定する

操作系統ごとの可否・パーミッションの全体像は「何ができて何ができないか」節の早見表を参照。
...
1. GitHub の Settings → Developer settings → Personal access tokens → Fine-grained tokens で新規トークンを作成する。**classic PAT は使わない**（最小の書き込みスコープ `repo` でも全リポジトリのコード読み書きを含んでしまい、漏洩時の被害が過大なため）。設定は用途に応じて選ぶ:
   ...
   - Repository permissions: 必要最小限のみ付与する（例: Issue連絡用なら `Issues: Read and write` のみ。自リポジトリのPRレビューまで行うなら追加で `Pull requests: Read and write`。`Contents` を `write` にすると push・PRマージもコンテナ内から実行できてしまうため、通常は付与しない — 付与しない限り push・マージ・Release作成はホスト側限定のまま維持される）
   - Expiration: 90日以下を推奨
...
3. ターゲットプロジェクトの `.claude-container.d/env` に、トークン本体ではなく**そのファイルへのパス**だけを書く: プライマリは `GH_TOKEN_FILE=~/.config/claude-container/<project>-gh-token`、セカンダリが必要なら `GH_TOKEN_SECONDARY_FILE=~/.config/claude-container/<project>-gh-token-secondary` も追加する。`.claude-container.d/env` はホスト固有のパスを含みうるため gitignore 対象であり、そもそもコミットされない。
4. ファイルが存在しない・通常ファイルでない場合は起動時にエラーで停止する（fail-closed）。パーミッションが `600` でない場合は警告が出る（両トークンとも同じ検証ロジック）。

トークンはホスト上のファイルとしてのみ扱われ、コンテナの `environment:` には渡らない（`podman inspect` 等にも露出しない）。コンテナ起動時に `entrypoint.sh` がプライマリのファイルの中身を読んで `GH_TOKEN` としてエクスポートし、`gh` CLI がこれを自動認識する（セカンダリはマウントのみで自動exportされない）。

**設定済みスコープの確認**: **現時点でセカンダリ（および任意のfine-grained PAT）の対象リポジトリ一覧を機械的に取得する手段は存在しない**。GitHub側にも対象リポジトリを一覧で返すAPIは無く（個人アカウント所有リポジトリ向けの同等APIは存在しない。組織所有リポジトリ限定の `GET /orgs/{org}/personal-access-tokens/{pat_id}/repositories` はGitHub App専用でPATでは使えない）、本プロジェクトもPAT設定変更への追従コストを避けるため対象リポジトリ自体を保持しない。したがって個別リポジトリ単位で疎通確認するしかない: `GH_TOKEN=$(cat <トークンファイル>) gh api /repos/<owner>/<repo>` を実行し、**private リポジトリに対してのみ** 200/404 がスコープ判定として機能する（200＝アクセス範囲内、404＝範囲外）。**public リポジトリはこの方法で判定できない**: GitHub は public リポジトリのメタデータ（`GET /repos/{owner}/{repo}` とその `permissions` フィールド）をトークンの `Repository access` 設定に関わらず常に200で返すため、公開リポジトリでは到達可否も `permissions` の値もスコープの証拠にならない（実機検証済み。詳細: `jj1xgo/claude-container#13`）。public リポジトリの実効スコープを確認したい場合は、実際に書き込み操作（`gh issue create` 等）を試すか、PAT設定画面（Web UI）の `Repository access` 一覧を直接確認すること。対象範囲の正本は常にPATの `Repository access` 設定側にある。

**トークンの更新**: 期限が近づいたら GitHub 側でトークンを再生成し、対応するファイルの中身を新しい文字列で上書きするだけでよい。いずれもビルド時に焼き込まれる設定ではなくランタイムマウントなので、リビルド（`-b`）は不要 — 次回起動時に読み直される。
...
**汎用シークレットディレクトリ（`SECRETS_DIR`）**: `GH_TOKEN_FILE`/`GH_TOKEN_SECONDARY_FILE` は固定2本という前提の機構だが、これはコンテナ側が本数・使い分けを制限しているだけで、fine-grained PAT は利用者が必要なだけ複数作成でき、どう使い分けるかは本来利用者の裁量である。MCP サーバー（後述の GitHub 公式リモート MCP レシピ参照）等、gh CLI 以外の用途でトークンを持ち込みたい場合にも対応するため、任意本数・任意名のトークンを配線できる汎用機構を用意している。

1. ホストにディレクトリを作り（例: `~/.config/claude-container/secrets.d/<project>`）、`chmod 700` する。中に置く各ファイルの**ファイル名がそのままコンテナ内の環境変数名になる**（`^[A-Za-z_][A-Za-z0-9_]*$` に合致しない名前は起動時に WARNING を出してスキップされる）。各ファイルは `chmod 600` し、中身はトークン文字列1行のみ（改行は自動で除去されるが、複数行の値は連結されてしまうため非対応）。各ファイルは実体（通常ファイル）として置くこと — コンテナにはこのディレクトリ単体がマウントされるため、ディレクトリ外を指すシンボリックリンクはコンテナ内でリンク先を解決できず、**警告なしにスキップされる**（既存のトークンファイルを流用したい場合はシンボリックリンクでなく値をコピーする）
2. ターゲットプロジェクトの `.claude-container.d/env` に `SECRETS_DIR=~/.config/claude-container/secrets.d/<project>` のようにパスを書く
3. ディレクトリが存在しない場合は起動時にエラーで停止する（fail-closed）。ディレクトリが `700` でない、または中のファイルが `600` でない場合は警告が出る（`GH_TOKEN_FILE` と同じ検証ロジック）
4. コンテナ内で、ディレクトリ内の各ファイルの中身がファイル名と同名の環境変数として export される。**既に環境に存在する変数名（`PATH` や、レガシー機構が export した `GH_TOKEN` 等）と衝突する場合は、既存の値を上書きせず警告を出してスキップする**（レガシー機構が設定されている場合はレガシー側が優先される）
5. **`noexport/` サブディレクトリ**: `SECRETS_DIR` 直下に `noexport/` という名前のサブディレクトリを作り、その中にファイルを置くと、そのファイルは自動 export されない（上記3の環境変数化ループは `[ -f ]` 判定でサブディレクトリ自体を素通りするため、意図的にスキップされる）。`GH_TOKEN_SECONDARY_FILE` と同じ「マウントするが自動 export しない」運用を、任意名のシークレットで使いたい場合に使う（例: 後述の `git push` 用トークン）。コンテナ内での読み取りは `~/.config/claude-container/secrets/noexport/<ファイル名>` を明示的に読む。パーミッション検証（`noexport/` 自体は `700`、中のファイルは `600`）・シンボリックリンク非対応の制約は直下のファイルと同様に適用される

これは汎用の環境変数注入機構であり、GitHub トークンに限らず任意のシークレットを持ち込める。トークン・シークレット同様、`podman inspect` 等には露出しない（`GH_TOKEN_FILE` と同じ理由 — `environment:` でなくファイルマウント経由で渡すため）。ただし持ち込んだ変数はコンテナ内の全プロセス（Claude 本体・hooks・任意の npm スクリプト等）から読めるため、1コンテナに持ち込むのはそのプロジェクトで実際に使う最小本数に留めること。

**GitHub 公式 MCP サーバーを使う場合のレシピ**: `gh` CLI でなく MCP 経由で GitHub を操作したい場合、`SECRETS_DIR` と組み合わせて次のように設定する。

1. ターゲットプロジェクトの `.claude-container.d/allowed-domains.txt` に `api.githubcopilot.com` を追加する（ビルド時焼き込みのため `-b` での再ビルドが必要）
2. `SECRETS_DIR` のディレクトリに PAT ファイルを置く（例: ファイル名 `GITHUB_MCP_PAT`）
...
**同じトークンを gh CLI（hooks 経由）と MCP の両方から使う場合の一本化配線**: セカンダリトークンを、hooks 等のシェル自動化（ファイル読み取り、非 export）と MCP（環境変数 export）の両方から使い回したい場合、実体ファイルを1本に保ったまま `GH_TOKEN_SECONDARY_FILE` と `SECRETS_DIR` の両方から同じホスト側パスを指せる（同一ファイルへの bind mount が2つ独立して張られるだけなので、前述のシンボリックリンク非対応の制約には抵触しない）。

```
GH_TOKEN_SECONDARY_FILE=~/.config/claude-container/secrets.d/<project>/GITHUB_MCP_PAT
SECRETS_DIR=~/.config/claude-container/secrets.d/<project>
```
...
**git push を使う場合（`SECRETS_DIR/noexport/GIT_PUSH_TOKEN`）**: デフォルトでは git のリモート操作のうち push は不可（後述「何ができて何ができないか」節）。対象リポジトリを限定した PAT を使い opt-in で有効化できる。

1. GitHub の Fine-grained PAT を作成する。Repository access は push 先リポジトリのみに限定し、Repository permissions で `Contents: Read and write` を付与する（push には `Contents: write` が必要 — 後述「プライマリ / セカンダリ PAT 対応表」参照）。GitHub 側の branch protection（レビュー必須化・force-push 禁止等）の併用を推奨する
2. トークン文字列を `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` という名前のファイルに保存し `chmod 600` する（`noexport/` サブディレクトリが無ければ作成し `chmod 700` する。前述「`noexport/` サブディレクトリ」参照）
3. ターゲットプロジェクトの `.claude-container.d/env` に `SECRETS_DIR=...` を設定する（他用途で設定済みなら追加設定不要 — 同じディレクトリの `noexport/` サブディレクトリを使うだけでよい）
4. `Dockerfile.claude` の変更を伴うため `-b` での再ビルドが必要

起動すると `entrypoint.sh` が `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` の存在を検知し `GIT_ASKPASS` を自動設定する（`GIT_PUSH_TOKEN` という環境変数自体は export されない）。以降、対象リポジトリへの `git push`（**HTTPS リモート限定** — SSH リモートには効かない）は、`git-askpass.sh` がトークンをファイルから都度読んで応答するため、追加の手動操作なしに通る。`git-askpass.sh` は github.com 宛の Username/Password プロンプトにのみ応答する fail-closed 設計で、他ホスト・想定外のプロンプトには応答しない。

このトークンは private リポジトリの fetch/pull にも有効になる（`Contents: Read` 相当を含むため）副作用がある点に注意。
...
コンテナ内の `git` と `gh` CLI は認証系統が完全に独立している。`git push` が失敗するのは権限不足ではなく credential helper を意図的に配線していないためであり、逆に `gh` は `GH_TOKEN` 環境変数を直接使うため git の設定とは無関係に通る。
...
| git リモート操作（`push` / `pull` / `fetch`） | git credential helper（既定で**未配線**。`GH_TOKEN` は git には渡らない。`SECRETS_DIR/noexport/GIT_PUSH_TOKEN` 設定時のみ `GIT_ASKPASS` 経由で配線される） | 既定では **push は不可**。**public リポジトリの fetch/pull は認証不要のため可**（private リポジトリの fetch/pull は不可）。`SECRETS_DIR/noexport/GIT_PUSH_TOKEN`（前述）を設定した場合のみ、対象リポジトリへの push（および同トークンでの private リポジトリの fetch/pull）が可能になる |
| `gh` CLI（プライマリ） | `GH_TOKEN` 環境変数 ＝ `entrypoint.sh` が自動 export | PAT のパーミッション・対象リポジトリの範囲内で可（下表） |
| `gh` CLI（セカンダリ） | コマンドごとの明示指定: `GH_TOKEN=$(cat /home/node/.config/claude-container/gh-token-secondary) gh ...` | 同上（自動 export はされない） |

**プライマリ / セカンダリ PAT 対応表**

| 項目 | プライマリ（`GH_TOKEN_FILE`） | セカンダリ（`GH_TOKEN_SECONDARY_FILE`） |
|---|---|---|
| 想定用途 | 主対象リポジトリへの日常操作（Issue 起票・自リポジトリの PR レビュー補助等） | それ以外のリポジトリへの Issue 連絡（クロスリポジトリ起票） |
| 自動 export | あり | なし（コマンドごとに明示指定） |
| 一般的に許可してよいパーミッション | `Issues: Read and write`。自リポジトリの PR レビュー補助まで行うなら `Pull requests: Read and write` を追加、必要なら `Contents: Read` | `Issues: Read and write` のみ |
| 持たせるべきでないパーミッション | `Contents: write`（push・PRマージ・Release 作成に直結するため非推奨） | 左に加え `Pull requests: write` も不要（Issue 連絡用途を超える） |
| 設定手順・スコープ確認 | 「利用側プロジェクトの設定」節参照 | 同左 |
...
`GH_TOKEN_FILE`/`GH_TOKEN_SECONDARY_FILE`/`SECRETS_DIR`（前述）を設定した場合、上記「許可済みサービス自体への送信」というリスクが受動的なものから能動的なものに変わる: プロンプトインジェクションや悪意あるパッケージがコンテナ内からトークンを読み取り、そのスコープ内で GitHub 等に書き込める。緩和策は各 fine-grained PAT のスコープ最小化（対象リポジトリ限定・短期限）で、被害を該当リポジトリでの操作に構造的に限定すること。`SECRETS_DIR` は汎用機構であるため、この能動的リスクは GitHub トークンに限らず持ち込んだ全シークレットに及ぶ（1コンテナに持ち込むのは実際に使う最小本数に留めること — 前述）。

`SECRETS_DIR/noexport/GIT_PUSH_TOKEN`（前述「git push を使う場合」）を設定した場合、能動的リスクは push にも及ぶ: プロンプトインジェクションや悪意あるパッケージが、対象リポジトリへの意図しないコミット・push を引き起こしうる。緩和策は他のトークン同様スコープ最小化（対象リポジトリ限定）に加え、GitHub 側の branch protection（force-push 禁止・レビュー必須化）を組み合わせること。

自リポジトリ向けにトークンへ `Pull requests: Read and write` を付与した場合の追加リスク:
...
| `GH_TOKEN_FILE` | (unset) | Absolute path on the host to a file holding a GitHub write token (primary, auto-exported as `GH_TOKEN`; see below) |
| `GH_TOKEN_SECONDARY_FILE` | (unset) | Absolute path on the host to a file holding a GitHub write token (secondary, not auto-exported — must be specified per command; see below) |
...
| `SECRETS_DIR` | (unset) | Host path to a generic secrets directory for bringing an arbitrary number/name of tokens into the container (see below) |
...
**Writing to GitHub (`GH_TOKEN_FILE` / `GH_TOKEN_SECONDARY_FILE`, legacy)**: To let the container write to GitHub via the `gh` CLI (issue creation, reviewing PRs on your own repo, etc.), wire up a token as follows. The more general `SECRETS_DIR` (below) can cover the same use case, but the "mounted but not auto-exported" secondary-style behavior is only available via `GH_TOKEN_SECONDARY_FILE`, so this section is kept alongside it for now.

**Important**: "where you place `.claude-container.d/env`" and "which repository you select under the PAT's `Repository access`" are two different things. The former is the project that **uses** the token (e.g. findsummits); the latter is the repository you're **writing to** (e.g. claude-container). To let findsummits write issues to claude-container, place `.claude-container.d/env` inside findsummits and select `claude-container` under `Repository access` — not the project that's doing the writing.

**Primary vs. secondary**: fine-grained PATs apply one permission set uniformly to every repository selected under a single token (you cannot grant different permissions to different repositories within the same token). So if you want "broader permissions (e.g. Pull requests) on your own repository, Issues-only elsewhere," you need two tokens:
- `GH_TOKEN_FILE` (primary): auto-exported as `GH_TOKEN` by `entrypoint.sh`, picked up automatically by the `gh` CLI. If one target/permission set is enough, this alone is sufficient
- `GH_TOKEN_SECONDARY_FILE` (secondary, optional): not auto-exported. Specify it explicitly before the command that needs it, e.g. `GH_TOKEN=$(cat /home/node/.config/claude-container/gh-token-secondary) gh issue create --repo <owner>/<repo> ...`

See the "What Works and What Doesn't" quick reference below for the full picture of availability and permissions by operation type.
...
1. Create a token under GitHub Settings → Developer settings → Personal access tokens → Fine-grained tokens. **Do not use a classic PAT** — even its narrowest write scope (`repo`) grants read/write on all your repositories, which is too much blast radius for a leaked token. Configure it according to your use case:
   ...
   - Repository permissions: grant only the minimum needed (e.g. `Issues: Read and write` for a communication channel; add `Pull requests: Read and write` if you also want the container reviewing PRs on your own repo. Do not set `Contents` to `write` — that would also enable push and PR merges from inside the container, so leave it unset or `Read`-only; push, merge, and release creation stay host-side as long as it is)
...
3. In the target project's `.claude-container.d/env`, set the path (not the token itself): `GH_TOKEN_FILE=~/.config/claude-container/<project>-gh-token` for the primary, and `GH_TOKEN_SECONDARY_FILE=~/.config/claude-container/<project>-gh-token-secondary` if you also need a secondary. `.claude-container.d/env` is gitignored (it may hold host-specific paths), so it's never committed in the first place.
4. Startup aborts (fail-closed) if a configured file doesn't exist or isn't a regular file; a warning is printed if its permissions aren't `600`. Same validation logic for both.

Tokens are only ever files on the host — never passed via the container's `environment:` (so they don't show up in `podman inspect`, etc.). At startup, `entrypoint.sh` reads the primary file's contents and exports it as `GH_TOKEN`, which the `gh` CLI picks up automatically (the secondary is mounted only — it is never auto-exported).

**Checking configured scope**: **There is currently no way to mechanically retrieve the full list of repositories a secondary (or any fine-grained) PAT covers.** GitHub itself has no API that lists a token's accessible repositories at once for personal-account tokens (the equivalent endpoint, `GET /orgs/{org}/personal-access-tokens/{pat_id}/repositories`, is organization-scoped and GitHub-App-only — it doesn't apply to PATs on personal accounts), and this project doesn't track the target repositories either, to avoid drifting out of sync with PAT config changes. So you have to check reachability per repository instead: run `GH_TOKEN=$(cat <token-file>) gh api /repos/<owner>/<repo>` — but the 200/404 result is only meaningful **for private repositories** (200 = in scope, 404 = out of scope). **This does not work for public repositories**: GitHub returns public repository metadata (`GET /repos/{owner}/{repo}` and its `permissions` field) with 200 regardless of the token's `Repository access` setting, so for a public repo neither reachability nor the `permissions` value is evidence of actual scope (verified empirically; see `jj1xgo/claude-container#13`). To check effective scope on a public repository, either attempt a real write operation (e.g. `gh issue create`) or check the `Repository access` list directly in the PAT settings UI. The source of truth for scope always remains the PAT's `Repository access` setting.

**Rotating a token**: When it's nearing expiration, regenerate it on GitHub's side and overwrite the contents of the corresponding file with the new string. Both are runtime mounts, not baked in at build time, so no rebuild (`-b`) is needed — the new contents are picked up on the next launch.
...
**Generic secrets directory (`SECRETS_DIR`)**: `GH_TOKEN_FILE`/`GH_TOKEN_SECONDARY_FILE` assume a fixed two-slot model, but that's a limitation the container imposes — fine-grained PATs let you create as many tokens as you want, and how you divide them up is really up to you. To support use cases beyond the `gh` CLI (e.g. an MCP server — see the GitHub official MCP recipe below), a generic mechanism lets you bring in an arbitrary number of arbitrarily-named tokens.

1. Create a directory on the host (e.g. `~/.config/claude-container/secrets.d/<project>`) and `chmod 700` it. **The filename of each file placed inside becomes the environment variable name inside the container** (names not matching `^[A-Za-z_][A-Za-z0-9_]*$` are skipped with a WARNING at startup). `chmod 600` each file; its contents should be a single-line token string (newlines are stripped automatically, so multi-line values get concatenated — not supported). Each file must be a regular file — since only this directory itself is mounted into the container, a symlink pointing outside it cannot be resolved inside the container and is **silently skipped with no warning** (to reuse an existing token file, copy its value instead of symlinking)
2. In the target project's `.claude-container.d/env`, set `SECRETS_DIR=~/.config/claude-container/secrets.d/<project>`
3. Startup aborts (fail-closed) if the directory doesn't exist. A warning is printed if the directory isn't `700` or a file inside isn't `600` (same validation logic as `GH_TOKEN_FILE`)
4. Inside the container, each file's contents are exported as an environment variable named after the file. **If the name collides with one already present in the environment (e.g. `PATH`, or `GH_TOKEN` if the legacy mechanism exported it), the existing value is left untouched and a warning is printed instead** (the legacy mechanism wins if both are configured)
5. **The `noexport/` subdirectory**: creating a subdirectory named `noexport/` directly under `SECRETS_DIR` and placing files inside it opts them out of auto-export (the export loop in step 3 uses `[ -f ]`, so it deliberately skips over subdirectories themselves). Use this when you want the same "mounted but not auto-exported" behavior as `GH_TOKEN_SECONDARY_FILE`, but for an arbitrarily-named secret (e.g. the `git push` token below). Inside the container, read it explicitly at `~/.config/claude-container/secrets/noexport/<filename>`. The same permission checks (`700` on `noexport/` itself, `600` on files inside) and the symlink limitation apply here too

This is a generic environment-variable-injection mechanism — it isn't limited to GitHub tokens. Like the other tokens, it never shows up in `podman inspect` etc. (same reasoning as `GH_TOKEN_FILE` — passed via a file mount, not `environment:`). That said, anything you bring in this way is readable by every process inside the container (Claude itself, hooks, any npm script), so only bring in the minimum set of secrets a given project actually needs.

**Recipe: using the GitHub official MCP server**: to talk to GitHub via MCP instead of the `gh` CLI, combine `SECRETS_DIR` with the following.
...
2. Place a PAT file in the `SECRETS_DIR` directory (e.g. named `GITHUB_MCP_PAT`)
...
**Recipe: sharing one token between the `gh` CLI (via hooks) and MCP**: if you want to reuse the same secondary token for both shell-based automation (file read, not exported) and MCP (exported as an env var), you can point both `GH_TOKEN_SECONDARY_FILE` and `SECRETS_DIR` at the same host path while keeping a single underlying file (this just creates two independent bind mounts of the same file, so it doesn't run into the symlink limitation noted above).
...
**Recipe: enabling `git push` (`SECRETS_DIR/noexport/GIT_PUSH_TOKEN`)**: by default, `push` doesn't work among git's remote operations (see "What Works and What Doesn't" below). You can opt in with a PAT scoped to specific repositories.

1. Create a Fine-grained PAT on GitHub. Limit Repository access to only the repository you're pushing to, and grant `Contents: Read and write` under Repository permissions (`push` requires `Contents: write` — see the "Primary vs. secondary PAT reference table" below). Pairing this with GitHub-side branch protection (required reviews, no force-push) is recommended
2. Save the token string to a file named `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` and `chmod 600` it (create the `noexport/` subdirectory and `chmod 700` it if it doesn't exist yet — see "The `noexport/` subdirectory" above)
3. Set `SECRETS_DIR=...` in the target project's `.claude-container.d/env` (no extra setup needed if it's already configured for another purpose — just add the `noexport/` subdirectory under the same directory)
4. Requires a `-b` rebuild, since it involves a `Dockerfile.claude` change

On launch, `entrypoint.sh` detects `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` and automatically sets `GIT_ASKPASS` (the `GIT_PUSH_TOKEN` environment variable itself is never exported). From then on, `git push` to the target repository (**HTTPS remotes only** — this does not work for SSH remotes) goes through without manual intervention, since `git-askpass.sh` reads the token from the file just in time on each prompt. `git-askpass.sh` is fail-closed: it only answers prompts addressed to github.com, and refuses any other host or unexpected prompt.
...
| git remote operations (`push` / `pull` / `fetch`) | git credential helper (**not wired** by default; `GH_TOKEN` is never passed to git. Wired via `GIT_ASKPASS` only when `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` is set) | By default, **push does not work**. **fetch/pull on public repositories works** (no auth required; private repos' fetch/pull don't work). Setting `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` (above) enables push to the target repository (and, as a side effect, fetch/pull on private repositories with the same token) |
| `gh` CLI (primary) | `GH_TOKEN` env var, auto-exported by `entrypoint.sh` | Works within the PAT's permissions and repository scope (see below) |
| `gh` CLI (secondary) | Explicit per-command: `GH_TOKEN=$(cat /home/node/.config/claude-container/gh-token-secondary) gh ...` | Same as above (never auto-exported) |

**Primary vs. secondary PAT reference table**

| Item | Primary (`GH_TOKEN_FILE`) | Secondary (`GH_TOKEN_SECONDARY_FILE`) |
|---|---|---|
| Intended use | Day-to-day operations on your main target repository (issue creation, reviewing PRs on your own repo, etc.) | Issue communication with other repositories (cross-repo filing) |
| Auto-exported | Yes | No (explicit per command) |
| Permissions generally fine to grant | `Issues: Read and write`. Add `Pull requests: Read and write` if it also reviews PRs on your own repo, and `Contents: Read` if needed | `Issues: Read and write` only |
| Permissions it should not have | `Contents: write` (enables push, PR merge, and release creation — not recommended) | Same, plus `Pull requests: write` is unnecessary (beyond issue-communication scope) |
| Setup and scope checking | See "Target Project Configuration" above | Same |
...
If `GH_TOKEN_FILE`/`GH_TOKEN_SECONDARY_FILE`/`SECRETS_DIR` (above) are set, the "exfiltration to allowed services themselves" risk above stops being passive: prompt injection or a malicious package running in the container can read a token and write to GitHub (or elsewhere) within its scope. The mitigation is scoping each fine-grained PAT down (single repository, short expiration), which structurally limits the blast radius to that repository. Since `SECRETS_DIR` is a generic mechanism, this active risk extends to every secret you bring in, not just GitHub tokens (again, bring in only the minimum set a given project actually needs — see above).

If `SECRETS_DIR/noexport/GIT_PUSH_TOKEN` (see "Recipe: enabling `git push`" above) is set, the active risk extends to push as well: prompt injection or a malicious package could cause unintended commits or pushes to the target repository. The mitigation is the same scoping (single repository) combined with GitHub-side branch protection (no force-push, required reviews).

Additional risk if a token grants `Pull requests: Read and write` on your own repository:
