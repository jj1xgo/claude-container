...
- **検査項目**: legacy トークン変数（`GH_TOKEN_FILE` 等）・`GITCONFIG_FILE`/`SECRETS_DIR` の存在とレイアウト（`noexport/` 残存等）・パーミッション・`packages.txt`/`requirements.txt`/`allowed-domains.txt` の有無・イメージの既ビルド有無（`podman` 利用可能な場合のみ）・MCP 監査ゲートの承認状態。これらは通常起動時の fail-closed ガードと同一の関数を共有しており、診断結果と実際の起動挙動が乖離しない設計になっている。
