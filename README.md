...
**GitHub への書き込み（`GH_TOKEN_FILE` / `GH_TOKEN_SECONDARY_FILE`）**: コンテナ内から `gh` CLI で GitHub に書き込む（Issue作成、自リポジトリのPRレビュー等）場合、以下の手順でトークンを配線する。
...
| git リモート操作（`push` / `pull` / `fetch`） | git credential helper（**未配線**。`GH_TOKEN` は git には渡らない） | **不可（全リポジトリで一律失敗）**。push・pull はホスト側で行う |
...
`GH_TOKEN_FILE`/`GH_TOKEN_SECONDARY_FILE`（前述）を設定した場合、上記「許可済みサービス自体への送信」というリスクが受動的なものから能動的なものに変わる: プロンプトインジェクションや悪意あるパッケージがコンテナ内からトークンを読み取り、そのスコープ内で GitHub に書き込める。緩和策は各 fine-grained PAT のスコープ最小化（対象リポジトリ限定・短期限）で、被害を該当リポジトリでの操作に構造的に限定すること。
...
**Writing to GitHub (`GH_TOKEN_FILE` / `GH_TOKEN_SECONDARY_FILE`)**: To let the container write to GitHub via the `gh` CLI (issue creation, reviewing PRs on your own repo, etc.), wire up a token as follows.
...
| git remote operations (`push` / `pull` / `fetch`) | git credential helper (**not wired**; `GH_TOKEN` is never passed to git) | **Does not work, for every repository**. Do push/pull on the host |
...
If `GH_TOKEN_FILE`/`GH_TOKEN_SECONDARY_FILE` (above) are set, the "exfiltration to allowed services themselves" risk above stops being passive: prompt injection or a malicious package running in the container can read a token and write to GitHub within its scope. The mitigation is scoping each fine-grained PAT down (single repository, short expiration), which structurally limits the blast radius to that repository.
