...
| git リモート操作（`push` / `pull` / `fetch`） | git credential helper（**未配線**。`GH_TOKEN` は git には渡らない） | **push は不可（全リポジトリで一律失敗）**。**public リポジトリの fetch/pull は認証不要のため可**（private リポジトリの fetch/pull は不可）。push はホスト側で行う |
...
| git remote operations (`push` / `pull` / `fetch`) | git credential helper (**not wired**; `GH_TOKEN` is never passed to git) | **push does not work, for every repository**. **fetch/pull on public repositories works** (no auth required; private repos' fetch/pull don't work). Do push on the host |
