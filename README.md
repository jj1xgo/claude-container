...
**プロジェクト固有の apt/pip パッケージ**は、ターゲットプロジェクトのルートに `.claude-container.d/` ディレクトリを置くことで指定できる。
...
どちらも任意。置かなければ claude-container 同梱のデフォルト（空のフォールバック）が使われる。

### アーキテクチャ

主要ファイルが連携して動作する。

- **`claude-container`**（bash）— エントリーポイント。絶対パスを解決し、`.env` を読み込み、`TZ` を自動検出した上で `CONTEXT` / `CLAUDE_CONTAINER_DIR` を設定して `podman compose run` に委譲する。ビルド前に、プロジェクト側の `.claude-container.d/packages.txt`・`.claude-container.d/requirements.txt`（無ければ claude-container 同梱のデフォルト）と `entrypoint.sh` を固定パス `.build-context/` に集約し、それを `BUILD_CONTEXT_DIR` としてビルドコンテキストに渡す（`-b` 指定時、またはイメージ未ビルド時のみ実行）。
...
- **`packages.txt`** / **`requirements.txt`** — claude-container 同梱のデフォルト apt/pip パッケージ一覧（フォールバック既定値）。プロジェクト側で上書きする場合は `.claude-container.d/` を使う（「利用側プロジェクトの設定」参照）。
...
**Project-specific apt/pip packages** can be declared by placing a `.claude-container.d/` directory at the root of the target project:
...
Both are optional. If absent, claude-container's bundled defaults (empty fallbacks) are used.

### Architecture

The main files work together:

- **`claude-container`** (bash) — Entry point. Resolves absolute paths, sources `.claude-container`, auto-detects `TZ`, sets `CONTEXT` / `CLAUDE_CONTAINER_DIR`, and delegates to `podman compose run`. Before building, it stages `entrypoint.sh` plus the project's `.claude-container.d/packages.txt` / `.claude-container.d/requirements.txt` (or claude-container's bundled defaults if absent) into a fixed path, `.build-context/`, and passes that as `BUILD_CONTEXT_DIR` (only when `-b` is passed, or when the image hasn't been built yet).
...
- **`packages.txt`** / **`requirements.txt`** — claude-container's bundled default apt/pip package lists (fallback values). Projects override these via `.claude-container.d/` (see "Target Project Configuration" above).
