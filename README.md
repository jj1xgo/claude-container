...
**設定済みスコープの確認**: fine-grained PAT には対象リポジトリ一覧をまとめて返す確認APIが無いため、個別リポジトリ単位で疎通確認する。`GH_TOKEN=$(cat <トークンファイル>) gh api /repos/<owner>/<repo>` を実行し、200なら到達可（アクセス範囲内）、404なら到達不可（範囲外、または存在しない）と判定できる。ただし未知のリポジトリを網羅的に確認する方法ではないため、対象範囲の正本は常にPATの `Repository access` 設定側にあると理解して運用する。
...
**Checking configured scope**: fine-grained PATs have no API that returns the full list of accessible repositories at once, so check reachability per repository. Run `GH_TOKEN=$(cat <token-file>) gh api /repos/<owner>/<repo>`: 200 means reachable (in scope), 404 means unreachable (out of scope, or the repo doesn't exist). This isn't exhaustive for unknown repositories, though — the source of truth for scope always remains the PAT's `Repository access` setting.
