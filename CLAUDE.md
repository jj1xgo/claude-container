...
#### 1. 計画を優先する
Plan Modeへの切り替え基準に該当する作業（グローバル CLAUDE.md 参照）は `.claude/plan-<slug>.md` に計画をまとめ、承認を得てから実装を始める（詳細は「計画ファイル・handover の扱い」節参照）。それ以外の軽微な実装タスクは GitHub issue として登録する（「タスク管理の流れ」節参照）。
...
- 実装前の計画確認・承認は従来どおり（Plan Mode 基準に該当する場合は `.claude/plan-<slug>.md`）
...
- `on-hold` — 長期保留。Open のまま維持し、本文に再検討トリガー（どうなったら再検討するか）を明記する
