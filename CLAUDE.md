...
### ミッションと守備範囲（issue 起票・受領・相談対応の前に照合する）

本リポジトリのミッションは「Claude Code を `--dangerously-skip-permissions` で安全に走らせるための、プロジェクト非依存なサンドボックス境界の提供・維持」。課題・要望・相談を本リポジトリの issue として扱う前に、次の基準へ機械的に照合する（「コンテナに関係ありそう」という直感で判定しない）:

**変更・修正の対象は、コンテナ境界そのもの（イメージビルド・起動フロー・ファイアウォール・マウント・持ち込みトークン・`.claude-container.d/` 設定機構）か？**

- **Yes** → 本リポジトリの issue として扱う
- **No — Claude の使い方・セッション運用・プロジェクト横断ワークフローの話** → 本リポジトリには起票せず、「その話は本プロジェクトのミッション外なので `jj1xgo/dotclaude-ops` 側に相談してください」と案内する。コンテナが**経路として関わる**だけではミッション内にならない（前例: コンテナ⇔ホスト間のセッション連携自動化 #20 は、変更対象がホスト側の運用機構だったため `dotclaude-ops#16` へ移管、2026-07-12）
- **No — 特定の利用側プロジェクト固有の話** → 当該プロジェクトへ案内する

適用の境界・ライフサイクル:

- `.claude/` 配下（session-start hook・filed-issues 等）はプロダクトの一部ではなく「本リポジトリ自身の開発運用」であり、その改善はここで扱ってよい。ただしそれを汎用化して全プロジェクトへ効かせたい要望は dotclaude-ops 側で扱う（前例: filed-issues 横展開 `dotclaude-ops#9`）
- 起票済みの issue が後からミッション外と判明した場合も同様に扱う: 移管先へ起票 → 元 issue に移管コメントを付けてクローズ → `.claude/filed-issues.txt` へ記録（追記・コミット・削除の運用は「タスク管理の流れ」節に従う）
...
- 起票前に「ミッションと守備範囲」（プロジェクト概要節）の判定を通す。ミッション外なら本リポジトリに起票しない
...
- **受領時もまず「ミッションと守備範囲」（プロジェクト概要節）で照合する**: ミッション外なら対応せず、dotclaude-ops または当該プロジェクトへの案内コメントを付けてクローズする
