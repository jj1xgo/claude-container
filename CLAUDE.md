...
対象プロジェクトのルートに置いた `.claude-container.d/env` ファイル（`KEY=VALUE` 形式の行）で設定し、起動前に自動で読み込まれる。値は最初の `=` 以降を文字どおりに扱う — クォート・展開・コマンド実行は一切解釈されない。このファイルは意図的に `source` しないため、悪意あるターゲットプロジェクトがホスト上でコードを実行することはできない。旧方式（単一ファイル `.claude-container`）との優先順位・フォールバックは下記「プロジェクト固有設定（`.claude-container.d/`）」節参照。
...
**ランタイム設定**: `.claude-container.d/env`（`KEY=VALUE` 形式）。上記「環境変数」節の読み込み対象そのもので、起動のたび毎回読み込まれビルド時には一切参照されない。旧方式の単一ファイル `.claude-container` も移行期間中は動作するが非推奨で、`.claude-container.d/env` が存在すれば常にそちらのみが採用され（マージしない）、両方存在する場合・旧方式のみの場合はそれぞれ警告が出る。既知の利用側プロジェクトの移行状況は `.claude/todo.md` の保留セクションで追跡する。すべて移行が確認でき次第、旧方式の読み込みコード・警告・ドキュメント記述を削除する。
